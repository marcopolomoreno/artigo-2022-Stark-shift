...
#define pontosDelta 25
#define pontosDeltaf 30
#define passoDelta 2
#define passoDeltaf 20
...
__global__ void Kernel(double* a11, double* a22, double* a33, double* a44, double* a12, double* b12, double* a23, double* b23, double* a14, double* b14,
	double* a43, double* b43, double* a13, double* b13, double* a24, double* b24, double* variavel, double* variavel2, double *Ad, double *Af)
{
...
int main()
{
	...
	double H[17];
	double rho14, rho13, azul;
	double v, pesoDoppler;

	double a[17][nucleos];
	double variavel[1], deltaf[1], Ad[1], Af[1];;
	int p, q;
	...
	double* dev_variavel[gpu], *dev_deltaf[gpu];
	...
	for (int kp = -pontosDeltaf; kp <= pontosDeltaf; kp++)   //loop nas frequências de Rabi
	{
		printf("\ndelta_f %d: ", kp);
		fprintf(arquivo[0], "\n"); fprintf(arquivo[1], "\n");

		deltaf[0] = (2 * Pi) * kp * passoDeltaf * 1e6;
		...
		for (p = -pontosDelta; p <= pontosDelta; p++)  //loop diodo
		{
			for (int pp = 0; pp <= gpu - 1; pp++)
			{
				...
				cudaMalloc((void**)&dev_deltaf[pp], 1 * sizeof(double));
				...
				cudaMemcpyAsync(dev_deltaf[pp], deltaf, 1 * sizeof(double), cudaMemcpyHostToDevice, stream[pp]);
			...
			for (int pp = 0; pp <= gpu - 1; pp++)
			{
				...
				Kernel << <blocks, threads, 0, stream[pp] >> > (dev_a11[pp], dev_a22[pp], dev_a33[pp], dev_a44[pp], dev_a12[pp], dev_b12[pp], dev_a23[pp], dev_b23[pp], dev_a14[pp],
					dev_b14[pp], dev_a43[pp], dev_b43[pp], dev_a13[pp], dev_b13[pp], dev_a24[pp], dev_b24[pp], dev_variavel[pp], dev_deltaf[pp], dev_Ad[pp], dev_Af[pp]);
			}
			...
			for (int pp = 0; pp <= gpu - 1; pp++)
			{
				...
				cudaMemcpyAsync(variavel, dev_deltaf[pp], 1 * sizeof(double), cudaMemcpyDeviceToHost, stream[pp]);
