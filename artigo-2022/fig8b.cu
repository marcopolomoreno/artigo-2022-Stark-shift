...
//26/05/2020: Gera gráfico tridimensional rho33, rho1 em função de delta_d  e  delta_f
//04/11/2021: Gera gráfico tridimensional rho33, rho1 em função de delta_d  e Omega_d
...
#define threads 32
#define gpu 1
#define arq 2		    //número de arquivo para salvar

#define kMax 200000
#define passoVelocidade 0.195
#define pontosDeltad 75
#define passoDeltad 2

#define pontosOmegad 60
#define passoOmegad 1
...
__constant__ double Af = (2 * Pi) * 0.6e6;    //em rad/s
...
__device__ double f(double a11, double a22, double a33, double a44, double a12, double b12, double a23, double b23, double a14, double b14,
					double a43, double b43, double a13, double b13, double a24, double b24, double deltad, double deltaa, int j, double v, double Ad)  //sistema de 4 níveis
{
...
__global__ void Kernel(double* a11, double* a22, double* a33, double* a44, double* a12, double* b12, double* a23, double* b23, double* a14, double* b14,
					   double* a43, double* b43, double* a13, double* b13, double* a24, double* b24, double* d_deltad, double* Ad)
{
	...
	for (k = 1; k <= kMax - 1; k++)    //abre loop de k (temporal)
	{
		for (j = 1; j <= 16; j++)
			k1[j] = f(a11[i], a22[i], a33[i], a44[i], a12[i], b12[i], a23[i], b23[i], a14[i], b14[i], a43[i], b43[i],
				      a13[i], b13[i], a24[i], b24[i], deltad, deltaa, j, v, Add);
		...
		for (j = 1; j <= 16; j++)
			k2[j] = f(a11[i] + k1[1] * h / 2, a22[i] + k1[2] * h / 2, a33[i] + k1[3] * h / 2, a44[i] + k1[4] * h / 2, a12[i] + k1[5] * h / 2, b12[i] + k1[6] * h / 2, a23[i] + k1[7] * h / 2, b23[i] + k1[8] * h / 2,
					 ...
					 a24[i] + k1[15] * h / 2, b24[i] + k1[16] * h / 2, deltad, deltaa, j, v, Add);
		...
		for (j = 1; j <= 16; j++)
			k3[j] = f(a11[i] + k2[1] * h / 2, a22[i] + k2[2] * h / 2, a33[i] + k2[3] * h / 2, a44[i] + k2[4] * h / 2, a12[i] + k2[5] * h / 2, b12[i] + k2[6] * h / 2, a23[i] + k2[7] * h / 2, b23[i] + k2[8] * h / 2,
				...
				a24[i] + k2[15] * h / 2, b24[i] + k2[16] * h / 2, deltad, deltaa, j, v, Add);
		...
		for (j = 1; j <= 16; j++)
			k4[j] = f(a11[i] + k3[1] * h, a22[i] + k3[2] * h, a33[i] + k3[3] * h, a44[i] + k3[4] * h, a12[i] + k3[5] * h, b12[i] + k3[6] * h, a23[i] + k3[7] * h, b23[i] + k3[8] * h, a14[i] + k3[9] * h, b14[i] + k3[10] * h,
				a43[i] + k3[11] * h, b43[i] + k3[12] * h, a13[i] + k3[13] * h, b13[i] + k3[14] * h, a24[i] + k3[15] * h, b24[i] + k3[16] * h, deltad, deltaa, j, v, Add);
...
int main()
{
	...
	arquivo[1] = fopen("dados-rho14.dat", "w");
	...
	double* dev_Ad[gpu]; double* dev_deltad[gpu],

	int bytes = nucleos * sizeof(double);
	cudaStream_t stream[gpu];

	for (int kp = 0; kp <= pontosOmegad; kp++)   //loop nas frequências de Rabi
	{
		printf("\ndelta_f %d: ", kp);
		fprintf(arquivo[0], "\n");
		fprintf(arquivo[1], "\n");

		h_Ad[0] = (2 * Pi) * kp * passoOmegad * 1e6;

		for (p = -pontosDeltad; p <= pontosDeltad; p++)  //loop diodo
		{
			for (int pp = 0; pp <= gpu - 1; pp++)
			{
				cudaSetDevice(pp);

				h_deltad[0] = (2 * Pi) * (gpu * p + pp) * passoDeltad * 1e6;
				printf("%d, ", gpu * p + pp);
				...
				cudaMalloc((void**)&dev_deltad[pp], 1 * sizeof(double));
				...
				cudaMemcpyAsync(dev_deltad[pp], h_deltad, 1 * sizeof(double), cudaMemcpyHostToDevice, stream[pp]);
				cudaMemcpyAsync(dev_Ad[pp], h_Ad, 1 * sizeof(double), cudaMemcpyHostToDevice, stream[pp]);
			}
			...
			for (int pp = 0; pp <= gpu - 1; pp++)
			{
				...
				Kernel <<< blocks, threads, 0, stream[pp] >>> (dev_a11[pp], dev_a22[pp], dev_a33[pp], dev_a44[pp], dev_a12[pp], dev_b12[pp], dev_a23[pp], dev_b23[pp], dev_a14[pp],
															   dev_b14[pp], dev_a43[pp], dev_b43[pp], dev_a13[pp], dev_b13[pp], dev_a24[pp], dev_b24[pp], dev_deltad[pp], dev_Ad[pp]);
			}
			...
			for (int pp = 0; pp <= gpu - 1; pp++)
			{
				...
				cudaMemcpyAsync(h_deltad, dev_deltad[pp], 1 * sizeof(double), cudaMemcpyDeviceToHost, stream[pp]);
				cudaMemcpyAsync(h_Ad, dev_Ad[pp], 1 * sizeof(double), cudaMemcpyDeviceToHost, stream[pp]);
				...
				fprintf(arquivo[0], "%g ", H[3]);
				fprintf(arquivo[1], "%g ", azul);
			}
		} //loop diodo
	...
	fclose(arquivo[0]); fclose(arquivo[1]);
